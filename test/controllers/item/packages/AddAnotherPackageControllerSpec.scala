...
class AddAnotherPackageControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators with MockitoSugar {

  private val formProvider = new AddAnotherFormProvider()

  private def form(viewModel: AddAnotherPackageViewModel): Form[Boolean] =
    formProvider(viewModel.prefix, viewModel.allowMore(frontendAppConfig))

  private val mode                        = NormalMode
  private lazy val addAnotherPackageRoute = routes.AddAnotherPackageController.onPageLoad(lrn, mode, itemIndex).url
  ...
  "AddAnotherPackage Controller" - {
    ...
    "must redirect to package type page when 0 package added" in {
      ...
      redirectLocation(result).value mustEqual
        controllers.item.packages.index.routes.PackageTypeController.onPageLoad(lrn, mode, itemIndex, Index(0)).url
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered " - {
      "when max limit not reached " in {
        ...
        setExistingUserAnswers(emptyUserAnswers.setValue(AddAnotherPackagePage(index), true))
    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to PackageType page at next index" in {
          ...
          redirectLocation(result).value mustEqual
            controllers.item.packages.index.routes.PackageTypeController.onPageLoad(lrn, mode, itemIndex, Index(listItems.length)).url
          ...
          userAnswerCaptor.getValue.get(AddAnotherPackagePage(index)).value mustEqual true
        }
      }
      ...
      "when no submitted" - {
        "must redirect to next page" in {
          ...
          val userAnswerCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
          verify(mockSessionRepository).set(userAnswerCaptor.capture())(any())
          userAnswerCaptor.getValue.get(AddAnotherPackagePage(index)).value mustEqual false
        }
      }
    }
