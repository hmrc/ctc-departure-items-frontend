    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to supply chain actor type page at next index" in {
          ...
          userAnswerCaptor.getValue.get(AddAnotherSupplyChainActorPage(index)).value mustEqual true
        }
      }
      ...
      "when no submitted" - {
        "must redirect to next page" in {
          ...
          val userAnswerCaptor: ArgumentCaptor[UserAnswers] = ArgumentCaptor.forClass(classOf[UserAnswers])
          verify(mockSessionRepository).set(userAnswerCaptor.capture())(any())
          userAnswerCaptor.getValue.get(AddAnotherSupplyChainActorPage(index)).value mustEqual false
