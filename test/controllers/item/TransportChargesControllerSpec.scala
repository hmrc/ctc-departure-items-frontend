...
class TransportChargesControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {
  ...
  "TransportCharges Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val view = injector.instanceOf[AddTransportChargesYesNoView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(AddTransportChargesYesNoPage(index), true)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request = FakeRequest(POST, transportChargesRoute)
