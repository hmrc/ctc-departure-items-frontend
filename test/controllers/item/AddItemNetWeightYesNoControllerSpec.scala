...
import views.html.item.AddItemNetWeightYesNoView
...
class AddItemNetWeightYesNoControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {
  ...
  "AddItemNetWeightYesNo Controller" - {

    "must return OK and the correct view for a GET" in {

      setExistingUserAnswers(emptyUserAnswers)

      val request = FakeRequest(GET, addItemNetWeightYesNoRoute)
      val result  = route(app, request).value

      val view = injector.instanceOf[AddItemNetWeightYesNoView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(AddItemNetWeightYesNoPage(itemIndex), true)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request = FakeRequest(POST, addItemNetWeightYesNoRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request   = FakeRequest(POST, addItemNetWeightYesNoRoute).withFormUrlEncodedBody(("value", ""))
