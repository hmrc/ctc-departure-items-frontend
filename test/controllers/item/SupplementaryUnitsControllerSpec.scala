...
class SupplementaryUnitsControllerSpec extends SpecBase with AppWithDefaultMockFixtures {
  ...
  "SupplementaryUnits Controller" - {

    "must return OK and the correct view for a GET" in {

      setExistingUserAnswers(emptyUserAnswers)

      val request = FakeRequest(GET, supplementaryUnitsRoute)

      val result = route(app, request).value

      val view = injector.instanceOf[SupplementaryUnitsView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(SupplementaryUnitsPage(itemIndex), validAnswer)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request    = FakeRequest(POST, supplementaryUnitsRoute).withFormUrlEncodedBody(("value", ""))
