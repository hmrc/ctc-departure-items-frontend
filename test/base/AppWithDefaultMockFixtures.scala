...
trait AppWithDefaultMockFixtures extends BeforeAndAfterEach with GuiceOneAppPerSuite with GuiceFakeApplicationFactory with MockitoSugar {
  ...
  protected val fakePackagesNavigatorProvider: PackagesNavigatorProvider =
    (mode: Mode, itemIndex: Index, packageIndex: Index) => new FakePackagesNavigator(onwardRoute, mode, itemIndex, packageIndex)
