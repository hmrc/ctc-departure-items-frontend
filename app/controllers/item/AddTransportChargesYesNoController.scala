...
import pages.item.AddTransportChargesYesNoPage
...
class AddTransportChargesYesNoController @Inject() (
  ...
  view: AddTransportChargesYesNoView
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, itemIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(AddTransportChargesYesNoPage(itemIndex)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            AddTransportChargesYesNoPage(itemIndex).writeToUserAnswers(value).updateTask().writeToSession(sessionRepository).navigateWith(navigator)
          }
        )
  }
}
