...
class AddAdditionalReferenceNumberYesNoController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, itemIndex: Index, additionalReferenceIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(AddAdditionalReferenceNumberYesNoPage(itemIndex, additionalReferenceIndex)) match {
        case None        => form
        case Some(value) => form.fill(value)
      }

      Ok(view(preparedForm, lrn, mode, itemIndex, additionalReferenceIndex))
  }
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            AddAdditionalReferenceNumberYesNoPage(itemIndex, additionalReferenceIndex).writeToUserAnswers(value).updateTask().writeToSession().navigate()
          }
        )
  }
}
