...
class AddShippingMarkYesNoController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, itemIndex: Index, packageIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(AddShippingMarkYesNoPage(itemIndex, packageIndex)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            AddShippingMarkYesNoPage(itemIndex, packageIndex).writeToUserAnswers(value).updateTask().writeToSession(sessionRepository).navigateWith(navigator)
          }
        )
  }
}
