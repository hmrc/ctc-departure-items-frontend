...
class TransportMethodOfPaymentController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, itemIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      ...
      Ok(view(preparedForm, lrn, TransportMethodOfPayment.values, mode, itemIndex))
  }
  ...
  def onSubmit(lrn: LocalReferenceNumber, mode: Mode, itemIndex: Index): Action[AnyContent] = actions.requireData(lrn).async {
    implicit request =>
      form
        .bindFromRequest()
        .fold(
          formWithErrors => Future.successful(BadRequest(view(formWithErrors, lrn, TransportMethodOfPayment.values, mode, itemIndex))),
          value => {
