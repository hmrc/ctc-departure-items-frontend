...
class CombinedNomenclatureCodeController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, itemIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(CombinedNomenclatureCodePage(itemIndex)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            CombinedNomenclatureCodePage(itemIndex).writeToUserAnswers(value).updateTask().writeToSession(sessionRepository).navigateWith(navigator)
          }
        )
  }
}
