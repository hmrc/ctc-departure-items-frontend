...
class ItemCountryOfDestinationController @Inject() (
  ...
  view: ItemCountryOfDestinationView
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, itemIndex: Index): Action[AnyContent] = actions.requireData(lrn).async {
    implicit request =>
      service.getCountries.map {
        countryList =>
          val form = formProvider("items.itemCountryOfDestination", countryList)
          val preparedForm = request.userAnswers.get(ItemCountryOfDestinationPage(itemIndex)) match {
            case None        => form
          ...
          form
            ...
            .fold(
              ...
              value => {
                ...
                ItemCountryOfDestinationPage(itemIndex).writeToUserAnswers(value).updateTask().writeToSession().navigate()
              }
            )
      }
  }
